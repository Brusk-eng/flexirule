	...
	async function evaluateDependencies(targetDoc, row = null, tableFieldname = null) {
		...
		for (const field of targetFields) {
			...
			if (field.mandatory_depends_on) {
				state.reqd = CoreUtils.eval_condition(field.mandatory_depends_on, evalContext)
					? 1
					: 0;
			}
			if (field.read_only_depends_on) {
				state.read_only = CoreUtils.eval_condition(field.read_only_depends_on, evalContext)
					? 1
					: 0;
			}
			...
			const isDynamic =
				...
				(typeof field.options === "string" &&
					(field.options.startsWith("doc.") || field.options.startsWith("vars.")));
