...
flexirule.validation.validate_variable_dependencies = function (actions, operation_metadata = {}) {
    ...
    for (const action of actions) {
        ...
        if (op_meta.reads_vars) {
            try {
                reads_vars = typeof op_meta.reads_vars === "string"
        ...
        if (Array.isArray(reads_vars)) {
            for (const varDef of reads_vars) {
                ...
                if (isRequired && varName && !available_vars.has(varName)) {
                    errors.push(
                        __("Action '{0}' requires variable '{1}' which is not produced by any prior action",
        ...
        if (op_meta.writes_vars) {
            try {
                writes_vars = typeof op_meta.writes_vars === "string"
 ...
 * @returns {Object} - { valid: boolean, errors: Array<string>, warnings: Array<string> }
 */
flexirule.validation.validate_flow_constraints = function (actions, operation_metadata = {}, edges = []) {
    const errors = [];
    const warnings = [];
    ...
    for (const action of actions) {
        ...
        if (op_meta.is_terminal === 1) {
            ...
            if (downstream.length > 0) {
                errors.push(
                    __("Action '{0}' is marked as terminal but has downstream actions. Remove connections to: {1}",
        ...
        if (op_meta.writes_to === "Database") {
            warnings.push(
                __("Action '{0}' writes directly to the database. This is a side-effect that cannot be rolled back.",
                    [action.action_label])
            );
