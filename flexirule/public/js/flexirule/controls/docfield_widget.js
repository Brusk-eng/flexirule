...
flexirule.DocFieldWidget = class DocFieldWidget {
     ...
     */
    render() {
        this.$input = $('<input class="form-control input-xs" placeholder="' + __('Select Field') + '">')
    ...
    _build_options() {
        ...
        // Valid fieldtypes
        const valid_types = [
            'Data', 'Link', 'Text', 'Phone', 'Email', 'Small Text',
            'Date', 'Datetime', 'Int', 'Float', 'Currency', 'Select',
            'Check', 'Dynamic Link', 'Percent', 'Rating', 'Duration'
        ];
...
flexirule.MultiDocFieldWidget = class MultiDocFieldWidget {
    ...
    render() {
        ...
        // Tags container
        this.$tags = $('<div class="multi-docfield-tags mb-2" style="display: flex; flex-wrap: wrap; gap: 4px;">');
        this.$wrapper.append(this.$tags);

        // Input
        this.$input = $('<input class="form-control input-xs" placeholder="' + __('Add Field') + '">');
    ...
    _render_tags() {
        ...
        this.value.forEach(fieldname => {
            ...
            const $tag = $(`<span class="badge badge-secondary" style="padding: 4px 8px; cursor: pointer;">
                ${label}
