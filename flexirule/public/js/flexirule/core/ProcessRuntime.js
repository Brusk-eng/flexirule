...
export default class ProcessRuntime extends BaseEngine {
            ...
            for (const field of fields) {
                if (field.fieldtype === "Table" && Array.isArray(data[field.fieldname])) {
                    for (const row of data[field.fieldname]) {
                        ...
                        await this.evaluate_dependencies(config, row, table_name, this.normalized_fields);
                        if (field.fields) {
    ...
    async optionsResolver(field) {
        if (field.fieldtype === "Autocomplete" || field.fieldtype === "DocField" || field.fieldtype === "MultiSelectList" || field.fieldtype === "MultiDocField") {
            ...
            if (!is_meta_only && ref !== "DocField" && typeof ref === "string" && ref.indexOf(".") === -1) {
                target = ref;
            }
