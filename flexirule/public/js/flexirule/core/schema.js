...
const SchemaUtils = {
        ...
        if (f.fieldtype === "Table") {
            ...
            if (!f._is_normalized) {
                ...
                for (const cf of children) {
                    normalizedChildren.push(await this.normalizeField(cf, config, context, optionsResolver));
                }
     ...
     */
    async evaluateDependencies(config, row, table_fieldname, fields, field_map, context, state_registry) {
        ...
        for (const field of target_fields) {
            ...
            if (!row && field.fieldtype === "Table") {
                ...
                await this._evaluate_single_field(field, eval_context, context_id, state_registry, changes);

                // Evaluate table columns (stored under table fieldname as shared state for all rows)
                await this.evaluateDependencies(config, { name: field.fieldname }, field.fieldname, fields, field_map, context, state_registry);
                continue;
            }
    ...
    async _evaluate_single_field(field, eval_context, context_id, state_registry, changes) {
        ...
        if (field.mandatory_depends_on) {
            newState.reqd = CoreUtils.eval_condition(field.mandatory_depends_on, eval_context) ? 1 : 0;
        }
        if (field.read_only_depends_on) {
            newState.read_only = CoreUtils.eval_condition(field.read_only_depends_on, eval_context) ? 1 : 0;
        }
