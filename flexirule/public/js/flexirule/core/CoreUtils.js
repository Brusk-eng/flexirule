...
export default {
		...
		for (const field of normalized_fields) {
			...
			if (
				...
				if (tableState.reqd && rows.length === 0) {
					errors.push(
						__("{0} requires at least one row", [field.label || field.fieldname])
					);
				}
	...
	_validate_table_rows(rows, table_field, dependency_states, errors) {
		...
		rows.forEach((row, idx) => {
			...
			const rowHelper =
				row_name && dependency_states[row_name] ? dependency_states[row_name] : {};
			...
			child_fields.forEach((cf) => {
				...
				if (fieldState.reqd) {
					...
					if (!has_value) {
						errors.push(
							__("Row #{0}: {1} is mandatory", [idx + 1, cf.label || cf.fieldname])
						);
					}
				}
			});
	...
	validate_json_schema(data, schema, path = "") {
		...
		if (expectedType && type !== expectedType) {
			if (!(expectedType === "integer" && Number.isInteger(data))) {
				errors.push(
					__("{0}: expected type {1}, got {2}", [path || "root", expectedType, type])
				);
				return errors;
			}
		}
		...
		if (type === "object" && schema.properties) {
			...
			Object.keys(schema.properties).forEach((prop) => {
				if (data[prop] !== undefined) {
					const sub = this.validate_json_schema(
						data[prop],
