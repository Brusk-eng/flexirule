            ...
            <div v-if="showConditionModal" class="condition-modal-overlay">
                <div class="condition-modal-content">
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-primary" @click="save_conditions">{{ __("Apply Conditions") }}</button>
                    </div>
                </div>
            </div>
        </Teleport>
...
const sidebar_title = computed(() => {
    ...
    return selectedNode.value.data?.action_label || 
...
const docFields = computed(() => {
    ...
    if (showOldDoc.value) {
        const oldFields = store.doc_fields
            ...
            .map(f => ({
                ...f,
                label: `old_doc.${f.fieldname} (${f.label.split('(')[1]?.replace(')', '') || f.label})`,
...
watch(showConditionModal, (val) => {
    if (val && selectedNode.value?.data) {
        ...
        let source = selectedNode.value.type === 'start' 
            ? selectedNode.value.data.trigger_condition
            : selectedNode.value.data.condition_json;
        
        let parsed = null;
        if (typeof source === 'string' && source.trim()) {
            try { parsed = JSON.parse(source); } catch(e) { parsed = null; }
        } else if (typeof source === 'object') {
            parsed = source;
        }
        
        currentConditions.value = parsed && (parsed.op || parsed.conditions)
