	...
	<div class="process-node-config">
		<div class="form-group relative">
			...
			<div v-if="showMethodSuggestions" class="suggestions-dropdown">
				...
				>
					<div class="d-flex justify-content-between align-items-center w-100">
						...
						<div class="method-badges">
							<span
								v-if="m.transactional"
								class="badge badge-warning"
								title="Transactional"
								style="font-size: 9px; padding: 2px 4px; margin-left: 2px"
								>Tx</span
							>
							<span
								v-if="m.side_effects === 'Pure'"
				...
				<div v-if="!filteredMethods.length" class="p-2 text-muted">
					{{ __("No methods found") }}
				</div>
			</div>
		</div>
		...
		<div class="form-group">
			<label
				...
				:class="{ 'text-muted': selectedMethod?.transactional }"
				:title="
					selectedMethod?.transactional
						? __('Transactional methods cannot run asynchronously')
						: ''
				"
			>
				<input
				...
				<span v-if="selectedMethod?.transactional" class="ml-1 text-warning"
					><i class="fa fa-lock"></i
				></span>
			</label>
			<div
				v-if="selectedMethod?.transactional"
				class="help-text text-danger mt-1"
...
const filteredMethods = computed(() => {
	return props.processMethods.filter(
		(m) =>
			m.method_name.toLowerCase().includes(methodSearch.value.toLowerCase()) ||
			(m.method_path &&
				m.method_path.toLowerCase().includes(methodSearch.value.toLowerCase()))
	);
});
