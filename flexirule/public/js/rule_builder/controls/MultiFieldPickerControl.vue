...
function removeField(fieldname) {
    emit("update:modelValue", selected.value.filter(f => f !== fieldname));
}
...
<style scoped>
.multi-field-picker { margin-bottom: 15px; }
.control-label { font-size: 12px; font-weight: 500; margin-bottom: 5px; display: block; }
.selected-fields { display: flex; flex-wrap: wrap; gap: 5px; margin-bottom: 8px; }
.field-tag {
    display: inline-flex; align-items: center; gap: 4px;
    padding: 3px 8px; background: var(--bg-light-gray);
...
.add-field-wrapper { position: relative; }
.field-dropdown {
...
.field-option:hover { background: var(--bg-light-gray); }
