<template>
    <div class="control-factory">
        ...
        <!-- Autocomplete -->
        <AutocompleteControl v-else-if="df?.fieldtype === 'Autocomplete'" :df="df" :modelValue="modelValue"
        ...
        <div v-else-if="['Int', 'Float', 'Currency', 'Percent'].includes(df?.fieldtype)" class="control frappe-control">
            <div v-if="df.label" class="control-label label" :class="{ reqd: df.reqd }">{{ __(df.label) }}</div>
        ...
        <!-- Date / Datetime -->
        <div v-else-if="['Date', 'Datetime'].includes(df?.fieldtype)" class="control frappe-control">
             <div v-if="df.label" class="control-label label" :class="{ reqd: df.reqd }">{{ __(df.label) }}</div>
            <input
        ...
        <!-- Text / Code / multiline -->
        <div v-else-if="['Text', 'Small Text', 'Text Editor', 'Code', 'JSON', 'HTML Editor', 'Markdown Editor'].includes(df?.fieldtype)" class="control frappe-control">
             <div v-if="df.label" class="control-label label" :class="{ reqd: df.reqd }">{{ __(df.label) }}</div>
             <textarea
