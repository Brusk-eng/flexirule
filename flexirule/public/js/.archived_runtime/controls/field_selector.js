...
flexirule.FieldSelector = class FieldSelector {
	...
	_build_field_options(doctype) {
		...
		// Add standard fields
		const std_fields = frappe.model.std_fields.filter(
			(f) => !frappe.model.no_value_type.includes(f.fieldtype)
		);
	...
	_on_field_select(item) {
		...
		// Build value array
		this.value = [source, doctype_or_key, item.fieldname, item.fieldtype, item.options || ""];
 ...
 */
frappe.ui.form.ControlFieldSelector = class ControlFieldSelector extends (
