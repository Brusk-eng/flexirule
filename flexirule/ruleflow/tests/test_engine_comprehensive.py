...
class TestRuleEngineComprehensive(FrappeTestCase):
    ...
    def create_test_rule(
        self, name, actions=None, doctype="ToDo", event="Validate", is_active=1
    ):
    ...
    def test_engine_execute_condition_true(self):
        ...
        engine = RuleEngine(rule)
        doc = frappe.get_doc(
            {"doctype": "ToDo", "description": "Test", "status": "Open"}
        )
        ...
        executed_actions = [
            ...
            if "Executing action" in entry["message"]
        ]
    ...
    def test_engine_execute_process_action(self):
        ...
        process_executed = any(
            "Validate Description" in action for action in executed_actions
        )
    ...
    def test_engine_execute_loop_action(self):
        ...
        doc = frappe.get_doc(
            {"doctype": "ToDo", "description": "Test with items", "status": "Open"}
        )
        doc.insert()
    ...
    def test_engine_execute_switch_action(self):
        ...
        process_executed = any(
            "Process Action" in action for action in executed_actions
        )
        self.assertTrue(process_executed)
    ...
    def test_engine_execute_wait_action(self):
        ...
        )  # At least 0.05s, accounting for processing time
    ...
    def test_engine_execute_with_error_handling_continue(self):
        ...
        fail_process_executed = any(
            "Fail Process" in action for action in executed_actions
        )
        after_error_executed = any(
            "After Error" in action for action in executed_actions
        )
    ...
    def test_engine_execute_with_cycle_detection(self):
        ...
        with self.assertRaises(Exception):
            rule = self.create_test_rule("Test Cycle Detection", actions=actions)
    ...
    def test_engine_execute_with_timeout(self):
        ...
        with patch("time.time", side_effect=TimeMock()):
            with self.assertRaises(BoltonTimeoutError):
                engine.execute(
                    doc, test_mode=False
                )  # Don't use test mode to enforce timeout
