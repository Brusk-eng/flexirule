...
frappe.ui.form.on("Rule Execution Log", {
    ...
    render_execution_path(frm) {
        ...
        let rows = path.map((step, idx) => {
            ...
            if (step.input && Object.keys(step.input).length) {
                details += `<div><span class="text-muted">Input:</span> ${format_v(step.input)}</div>`;
            }
            if (step.output) {
                // If output is string representation of list like "[{'name': ...}]"
                details += `<div><span class="text-muted">Output:</span> ${format_v(step.output)}</div>`;
            }
            if (step.error) {
                details += `<div class="text-danger"><strong>Error:</strong> ${format_v(step.error)}</div>`;
            }
    ...
    render_context_snapshot(frm) {
        ...
        let sections = Object.keys(context).map(key => {
            let val = context[key];
            let json_html = `<pre style="max-height: 300px; overflow: auto;">${JSON.stringify(val, null, 2)}</pre>`;
            return `
