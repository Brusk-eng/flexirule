            ...
            result["parent_fields"].append(
                {
                    ...
                    "options": df.options,
                    "description": f"{df.fieldtype}"
                    + (f" → {df.options}" if df.options else ""),
                }
            )
        ...
        for df in meta.fields:
            if df.fieldtype == "Table" and df.options:
                child_fields = get_child_table_fields(
                    df.options, df.fieldname, allowed_types, excluded_types
                )
                if child_fields:
...
def get_schema_field_options(schema_field, parent_doctype, current_values=None):
    ...
    current = (
        json.loads(current_values)
        if isinstance(current_values, str)
        else (current_values or {})
    )
...
def import_rule(import_data, overwrite=False):
    ...
    overwrite = (
        frappe.parse_json(overwrite) if isinstance(overwrite, str) else overwrite
    )
...
def get_action_context_schema(rule_name, action_id):
    ...
    while queue:
        ...
        for action in rule.actions:
            ...
            if (
                action.next_step_if_true == current_id
                or action.next_step_if_false == current_id
            ):
