...
def find_similar_records(context, config):
	...
	try:
		candidates = frappe.get_all(doc.doctype, filters=filters, fields=fieldnames, limit=1000)
	...
	for cand in candidates:
		...
		for f in fields_config:
			...
			score = matcher.get_score(f.get("algorithm", "Fuzzy"), doc.get(fname), cand.get(fname), f)
...
def find_duplicates_in_child_table(context, config):
	...
	vals = [r.get(search_field) for r in (doc.get(child_field) or []) if r.get(search_field)]
	if not vals:
		return []
