...
def send_email(context, config=None, **kwargs):
    ...
    rendered_subject = frappe.render_template(
        subject, {"doc": doc, "frappe": frappe, "context": context}
    )
    rendered_message = frappe.render_template(
        message, {"doc": doc, "frappe": frappe, "context": context}
    )
    ...
    if attach_doc:
        try:
            ...
            frappe.log_error(
                f"Notification: Failed to attach PDF: {e}", "Notification Error"
            )
...
def add_comment(context, config=None, **kwargs):
    ...
    rendered_text = frappe.render_template(
        comment_text, {"doc": doc, "frappe": frappe, "context": context}
    )
...
def send_via_provider(context, config=None, **kwargs):
    ...
    for app in frappe.get_installed_apps():
        app_providers = frappe.get_hooks(
            "flexirule_notification_providers", app_name=app
        )
        if app_providers:
    ...
    if provider_name not in providers:
        frappe.log_error(
            f"Notification: Provider '{provider_name}' not found.", "Notification Error"
        )
        return False
