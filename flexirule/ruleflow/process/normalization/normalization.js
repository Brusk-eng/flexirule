...
const PROFILES = {
	default: ["trim", "unicode_normalize", "casefold", "translate_chars", "remove_extra_spaces"],
	arabic_strict: [
		"trim",
		"unicode_normalize",
		"casefold",
	...
	email: ["trim", "lowercase", "email_normalize"],
	phone: ["numeric_only"],
	slug: ["trim", "lowercase", "remove_punctuation", "remove_extra_spaces", "slug"],
};
		...
		},
		{
			...
			get_config_fields: (ctx) => {
				...
				return [
					...
					{ fieldtype: "Column Break" },
					{
						...
						label: __("Output Variable Name"),
						fieldtype: "Data",
						description: __(
							"Key to store in 'vars'. Defaults to normalized_{source_field}"
						),
					},
					{
						fieldtype: "Section Break",
			...
			get_output_schema: (config, ctx) => {
				const key =
					config.context_key ||
					(config.source_field
						? `normalized_${config.source_field}`
						: "normalized_value");
				return [
					{
						label: key,
		...
		},
		{
			...
			get_output_schema: (config, ctx) => {
				if (config.store_in_context) {
					...
					return fields.map((f) => {
						const key =
							f.target_field || (f.fieldname ? `normalized_${f.fieldname}` : "var");
	...
	return [
		{
			label: __("Trim"),
			value: "trim",
			description: __("Remove leading and trailing whitespace"),
		},
		{
			label: __("Lowercase"),
			value: "lowercase",
			description: __("Convert all characters to lowercase"),
		},
		{
			label: __("Uppercase"),
			value: "uppercase",
			description: __("Convert all characters to uppercase"),
		},
		{
			label: __("Casefold"),
			value: "casefold",
			description: __(
				"Aggressive lowercase for caseless matching (supports special Unicode characters)"
			),
		},
		{
			label: __("Unicode Normalize"),
			value: "unicode_normalize",
			description: __(
				"Normalize Unicode characters to NFKD form for consistent representation"
			),
		},
		{
			label: __("Base Character Form"),
			value: "translate_chars",
			description: __("Translate Arabic/Special characters to their standard base forms"),
		},
		{
			label: __("Remove Spaces"),
			value: "remove_spaces",
			description: __("Remove all spaces from the text"),
		},
		{
			label: __("Remove Extra Spaces"),
			value: "remove_extra_spaces",
			description: __("Replace multiple consecutive spaces with a single space"),
		},
		{
			label: __("Digits Only"),
			value: "numeric_only",
			description: __("Keep only numeric characters (removes letters and symbols)"),
		},
		{
			label: __("Remove Punctuation"),
			value: "remove_punctuation",
			description: __("Remove all punctuation and special characters"),
		},
		{
			label: __("Remove Numbers"),
			value: "remove_numbers",
			description: __("Remove all numeric characters"),
		},
		{
			label: __("Alphanumeric Only"),
			value: "alphanumeric_only",
			description: __("Keep only letters and numbers (removes symbols and spaces)"),
		},
		{
			label: __("Slug"),
			value: "slug",
			description: __(
				"Convert text to a URL-friendly slug (lowercase, alphanumeric, and hyphens)"
			),
		},
		{
			label: __("Title Case"),
			value: "title_case",
			description: __("Capitalize the first letter of each word"),
		},
		{
			label: __("Normalize Name"),
			value: "name_normalize",
			description: __("Normalize name to proper case (e.g. ALI AHMED -> Ali Ahmed)"),
		},
		{
			label: __("Email Normalize"),
			value: "email_normalize",
			description: __(
				"Standardize email format (lowercase, remove dots/plus in Gmail addresses)"
			),
		},
		{
		...
		},
		{
			...
			value: "tax_id_clean",
			description: __("Clean government IDs by removing spaces and hyphens"),
		},
		{
			label: __("Standardize Date"),
			value: "standard_date",
			description: __("Attempt to convert text date to standard ISO format (YYYY-MM-DD)"),
		},
	];
}
...
function get_profile_field(is_table_target = true) {
	return {
		...
		options: [
			{ label: "Custom", value: "" },
			{ label: "Default Cleanup", value: "default" },
			{ label: "Arabic Strict", value: "arabic_strict" },
			{ label: "Email Optimization", value: "email" },
			{ label: "Phone/Numbers only", value: "phone" },
			{ label: "Web Slug", value: "slug" },
		],
