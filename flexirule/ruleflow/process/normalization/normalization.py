...
def execute(context, func=None, config=None):
    ...
    if func not in _OPERATIONS:
        frappe.throw(
            _("Unknown operation: {0}. Available: {1}").format(
...
TRANSFORMATIONS: dict[str, callable] = {
    ...
    "unicode_normalize": lambda x: (
        unicodedata.normalize("NFKD", x) if isinstance(x, str) else x
    ),
    "translate_chars": lambda x: (
        x.translate(TRANSLATION_TABLE) if isinstance(x, str) else x
    ),
    "remove_spaces": lambda x: x.replace(" ", "") if isinstance(x, str) else x,
    "remove_extra_spaces": lambda x: (
        re.sub(r"\s+", " ", x).strip() if isinstance(x, str) else x
    ),
    "remove_punctuation": lambda x: (
        re.sub(r"[^\w\s]", "", x) if isinstance(x, str) else x
    ),
    ...
    "slug": lambda x: (
        re.sub(r"[^\w\s-]", "", x).strip().lower().replace(" ", "-")
        if isinstance(x, str)
        else x
    ),
    ...
    "currency_to_number": lambda x: (
        re.sub(r"[^\d.]", "", x) if isinstance(x, str) else x
    ),
    "tax_id_clean": lambda x: re.sub(r"[\s-]", "", x) if isinstance(x, str) else x,
    "standard_date": lambda x: (
        str(frappe.utils.getdate(x)) if isinstance(x, str) and x else x
    ),
}
...
def apply_transformations(value, transformations):
    ...
    if (
        ...
        and transformations
        and isinstance(transformations[0], dict)
    ):
        transformations = [
            t.get("transformation") for t in transformations if t.get("transformation")
        ]
    ...
    for (
        ...
        transformations
    ):  # Renamed 'transform' to 'transform_name' to avoid conflict with TRANSFORMATIONS
...
def normalize_multiple_fields(context, config):
    ...
    for item in field_config:
        ...
        if isinstance(transformations, str):
            transformations = [
                t.strip() for t in transformations.split("\n") if t.strip()
            ]
