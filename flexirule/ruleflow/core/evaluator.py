...
class ConditionEvaluator:
    ...
    def _evaluate_collection(self, node: dict, doc, row=None) -> bool:
        ...
        if logic == "any":
            ...
            return (
                all(self._evaluate_group([where], doc, r) for r in rows)
                if rows
                else True
            )
        elif logic == "none":
    ...
    def _evaluate_single(self, condition: dict, doc, row=None) -> bool:
        ...
        try:
            ...
            if (
                ...
                if op in ["==", "!="]:
                    is_link_tuple = True
                elif op in ["in", "not_in", "not in"] and isinstance(
                    right[1], (list, tuple)
                ):
                    is_link_tuple = True
