...
def check_rule_permission(rule_doc, throw=True):
    ...
    if not safe_get(rule_doc, "is_active"):
        if throw:
            frappe.throw(
                _("Rule {0} is disabled (inactive)").format(
    ...
    if not frappe.has_permission(target_doctype, "write"):
        if throw:
            frappe.throw(
                _("You need write permission on {0} to execute this rule").format(
                    target_doctype
                ),
                frappe.PermissionError,
            )
...
def can_modify_rule(rule_doc, throw=True):
    ...
    if throw:
        frappe.throw(
            _("You don't have permission to modify this rule"), frappe.PermissionError
        )
    return False
